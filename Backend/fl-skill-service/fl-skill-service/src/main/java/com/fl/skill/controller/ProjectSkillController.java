...
public class ProjectSkillController {
    ...
    @GetMapping("/")
    public ResponseEntity<CommonResponse> getProjectSkills()
    {
        try {
            List<ProjectSkills> projects = new ArrayList<>();
            projectSkillsRepo.getAllProjectId().forEach(projects::add);
            List<ProjectSkillList> lst = new ArrayList<ProjectSkillList>();
            if(!projects.isEmpty())
            {
                for(int i=0;i<projects.size();i++)
                {
                    ProjectSkillList ps;
                    List<ProjectSkills> skillRes=new ArrayList<ProjectSkills>();
    ...
    @GetMapping("/{id}")
    public ResponseEntity<CommonResponse> getProjectSkillsById(@PathVariable("id") int id)
    {
        try {
            List<ProjectSkills> projects = new ArrayList<>();
